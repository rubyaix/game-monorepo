---
name: "imagegen"
description: "Use when the user asks to generate or edit images via the Gemini Image API (for example: generate image, edit/inpaint/mask, background removal or replacement, transparent background, product shots, concept art, covers, or batch variants); write custom Python scripts using `google-genai` and require `GEMINI_API_KEY` for live calls."
---


# Image Generation Skill

Generates or edits images for the current project (e.g., website assets, game assets, UI mockups, product mockups, wireframes, logo design, photorealistic images, infographics). Defaults to `imagen-3.0-generate-001` (or equivalent) and uses the Gemini API. Avoid using the bundled CLI (`scripts/image_gen.py`) as it is tailored for OpenAI; instead, write your own lightweight Python scripts using the `google-genai` SDK.
...
## Environment
- `GEMINI_API_KEY` must be set for live API calls.

If the key is missing, give the user these steps:
1. Create an API key in Google AI Studio: https://aistudio.google.com/app/apikey
2. Set `GEMINI_API_KEY` as an environment variable in their system.
...
## Defaults & rules
- Use `imagen-3.0-generate-001` (or latest available Gemini image model) unless the user explicitly asks for a different model.
- Assume the user wants a new image unless they explicitly ask for an edit.
- Require `GEMINI_API_KEY` before any live API call.
- Use the official Google GenAI Python SDK (`google-genai` package) for all API calls; do not use raw HTTP.
- If the user requests edits, use the appropriate Gemini image editing methods.
- Write new lightweight Python scripts using the `google-genai` SDK to interact with the model instead of using the bundled CLI (`scripts/image_gen.py`), as the CLI is tailored for OpenAI.
- Never modify `scripts/image_gen.py`. If you need functionality, write a new script in the `tmp/imagegen/` directory instead.
